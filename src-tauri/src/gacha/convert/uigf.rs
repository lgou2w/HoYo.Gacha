...
impl GachaConverter for UIGFGachaConverter {
  ...
  fn deconvert(&self, provided: Self::Provided) -> Result<Vec<GachaRecord>, Self::Error> {
    ...
    for provide in provided {
      ...
      let rank_type = provide
        .rank_type
        .ok_or(UIGFGachaConverterError::RequiredFields("rank_type"))?
        .parse::<u8>()
        .map_err(|inner| UIGFGachaConverterError::FieldParseInt {
          field: "rank_type",
          inner,
...
impl UIGFGachaRecordsWriter {
  pub const UIGF_VERSION: &'static str = "v2.2"; // TODO: -> UIGF v2.4
  ...
  fn construct_uigf_info(&self) -> UIGFInfo {
    ...
    UIGFInfo {
      ...
      export_app: Self::EXPORT_APP.to_owned(),
      export_app_version: Self::EXPORT_APP_VERSION.to_owned(),
...
mod tests {
  ...
  async fn test_writer() -> Result<(), UIGFGachaConverterError> {
    ...
    assert_eq!(uigf.info.export_app, UIGFGachaRecordsWriter::EXPORT_APP);
    assert_eq!(
      uigf.info.export_app_version,
      UIGFGachaRecordsWriter::EXPORT_APP_VERSION
    );
    ...
    assert_eq!(item.item_type, record.item_type);
