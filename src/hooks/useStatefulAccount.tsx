...
export const StatefulAccountContext = React.createContext<
  StatefulAccount | undefined
>(undefined);
...
const LocalStorageSelectedAccountUid = Object.freeze({
  get(facet: AccountFacet) {
    return localStorage.getItem(
      `${QueryPrefix}:${facet}:${SelectedAccountUidKey}`
    );
  },
  ...
  remove(facet: AccountFacet) {
    localStorage.removeItem(`${QueryPrefix}:${facet}:${SelectedAccountUidKey}`);
  },
});

const statefulAccountQueryFn: FetchQueryOptions<StatefulAccount>["queryFn"] =
  async (context) => {
...
function createQuery(
  facet: AccountFacet
): FetchQueryOptions<StatefulAccount> & { queryKey: QueryKey } {
  return {
...
export function useStatefulAccountContext() {
  ...
  if (!context) {
    throw new Error(
      "useStatefulAccountContext must be used within a StatefulAccountContext.Provider"
    );
  } else {
    return context;
  }
...
export function useSetSelectedAccountFn() {
  ...
  return React.useCallback(
    (uid: AccountUid) => {
      ...
      queryClient.setQueryData<StatefulAccount>(
        [QueryPrefix, statefulAccount.facet],
        (prev) => {
          return (
            prev &&
            produce(prev, (draft) => {
...
export function useCreateAccountFn() {
  ...
  return React.useCallback(
    async (payload: Omit<CreateAccountPayload, "facet">) => {
      const selectedAccountUid = statefulAccount.selectedAccountUid;
      const account = await PluginStorage.createAccount({
        ...payload,
        facet: statefulAccount.facet,
      });

      if (!selectedAccountUid)
        LocalStorageSelectedAccountUid.set(statefulAccount.facet, account.uid);
      queryClient.setQueryData<StatefulAccount>(
        [QueryPrefix, statefulAccount.facet],
        (prev) => {
          return (
...
function createUseUpdateAccountFn<Args extends unknown[]>(
  caller: (...args: Args) => Promise<Account>
) {
  return function () {
    const queryClient = useQueryClient();
    const statefulAccount = useStatefulAccountContext();
...
export const useUpdateAccountGameDataDirFn = createUseUpdateAccountFn(
  PluginStorage.updateAccountGameDataDir
);
export const useUpdateAccountGachaUrlFn = createUseUpdateAccountFn(
  PluginStorage.updateAccountGachaUrl
);
export const useUpdateAccountPropertiesFn = createUseUpdateAccountFn(
  PluginStorage.updateAccountProperties
);
          ...
          return (
            ...
            produce(prev, (draft) => {
              ...
              if (statefulAccount.selectedAccountUid === uid) {
                draft.selectedAccountUid =
                  Object.keys(draft.accounts)[0] ?? null;
              }
              if (!draft.selectedAccountUid) {
                LocalStorageSelectedAccountUid.remove(statefulAccount.facet);
              } else {
                LocalStorageSelectedAccountUid.set(
                  statefulAccount.facet,
                  draft.selectedAccountUid
                );
              }
            })
